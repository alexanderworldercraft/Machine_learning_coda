...
def load_image_features(pet_id, image_dir, model, image_size=(224, 224)):
    ...
    for img_path in image_paths:
        ...
        if "._" in os.path.basename(img_path):
            print(f"Fichier ignoré : {img_path}")
            continue
    ...
    # Si plusieurs images existent, prendre la moyenne des caractéristiques
    if len(image_features) > 0:
        return np.mean(image_features, axis=0)
    else:
        # Si aucune image n'est disponible ou que des erreurs ont été rencontrées
...
submission_path = './asset/data/submission_image_GradientBoostingClassifier_tensorflow.csv'
